...
class Door(WorldObj):
    def __init__(self, color, is_open=False, is_locked=False, reward=None):
    ...
    def toggle(self, env, pos):
        ...
        if self.reward is None:
            ...
            return True
        # When reward is set: Doors cannot be closed anymore
        else:
            if self.is_locked:
                raise NotImplementedError()
            # Can't close anymore once open to avoid infinite reward generation
            if self.is_open:
                return False, 0
            self.is_open = True
...
class Grid:
    ...
    # def encode(self, vis_mask=None):
    #     """
    #     Produce an image representation of the grid (array) as well as a compact numpy encoding of the grid (encoding)
    #     """

    #     if vis_mask is None:
    #         vis_mask = np.ones((self.width, self.height), dtype=bool)

    #     array = np.zeros((self.width * AGENT_CELL_PIXELS, self.height * AGENT_CELL_PIXELS, 3), dtype='float32')
    #     for i in range(self.width):
    #         for j in range(self.height):
    #             if vis_mask[i, j]:
    #                 v = self.get(i, j)

    #                 for a in range(AGENT_CELL_PIXELS):
    #                     for b in range(AGENT_CELL_PIXELS):
    #                         if v is None:
    #                             array[i*AGENT_CELL_PIXELS+a, j*AGENT_CELL_PIXELS+b, 0] = COLORS['black'][0] / 255.
    #                             array[i*AGENT_CELL_PIXELS+a, j*AGENT_CELL_PIXELS+b, 1] = COLORS['black'][1] / 255.
    #                             array[i*AGENT_CELL_PIXELS+a, j*AGENT_CELL_PIXELS+b, 2] = COLORS['black'][2] / 255.
    #                         else:
    #                             array[i*AGENT_CELL_PIXELS+a, j*AGENT_CELL_PIXELS+b, 0] = COLORS[v.color][0] / 255.
    #                             array[i*AGENT_CELL_PIXELS+a, j*AGENT_CELL_PIXELS+b, 1] = COLORS[v.color][1] / 255.
    #                             array[i*AGENT_CELL_PIXELS+a, j*AGENT_CELL_PIXELS+b, 2] = COLORS[v.color][2] / 255.

    #     encoding = np.zeros((self.width, self.height, 3), dtype='uint8')
    #     for i in range(self.width):
    #         for j in range(self.height):
    #             if vis_mask[i, j]:
    #                 v = self.get(i, j)
    ...
    #                 if v is None:
    #                     encoding[i, j, 0] = OBJECT_TO_IDX['empty']
    #                     encoding[i, j, 1] = 0
    #                     encoding[i, j, 2] = 0
    #                 else:
    #                     # State, 0: open, 1: closed, 2: locked
    ...
    #                     encoding[i, j, 0] = OBJECT_TO_IDX[v.type]
    #                     encoding[i, j, 1] = COLOR_TO_IDX[v.color]
    #                     encoding[i, j, 2] = state
...
class MiniGridEnv(gym.Env):
    ...
    def __init__(
        ...
        self.dro = False
        self.worst_env_seeds = []
        self.train_mode = True
        self.num_train_environments = 1000000000000000000
        self.num_test_environments = 1000
    ...
    def set_dro_mixing_coefficient(self, mixing_coeff):
        self.dro_mixing_coefficient = mixing_coeff

    def add_dro_seed(self, seed):
        if self.dro:
            if seed not in self.worst_env_seeds:
                self.worst_env_seeds.append(seed)
            if len(self.worst_env_seeds) > self.max_env_seeds:
                self.worst_env_seeds.pop(0)
    
    def remove_dro_seed(self, seed):
        if self.dro and seed in self.worst_env_seeds:
            self.worst_env_seeds.remove(seed)

    def set_dro_seeds(self, seeds):
        if self.dro:
            self.worst_env_seeds = seeds

    def get_dro_seeds(self):
        return self.worst_env_seeds
    ...
    def reset(self):
        ...
        # If the number is less than self.dro_mixing_coefficient, randomly select an environment from the worst environments.
        # If the number is >= self.dro_mixing_coefficient, randomly select an environment from the full distribution.
        self.max_env_seeds = int(.2 * self.num_train_environments)

        if self.train_mode:
            r = np.random.uniform(0.0, 1.0)
            if r < self.dro_mixing_coefficient and len(self.worst_env_seeds) > 0:
                self.env_seed = self.worst_env_seeds[np.random.randint(0, len(self.worst_env_seeds))]
            else:
                self.env_seed = np.random.randint(0, self.num_train_environments)
        else:
            self.eval_seed += 1
            self.eval_seed = self.eval_seed % self.num_test_environments
            self.env_seed = self.num_train_environments + (self.eval_seed % self.num_test_environments)
            # self.env_seed = np.random.randint(self.num_train_environments, self.num_train_environments + self.num_test_environments)
    ...
    def step(self, action):
        ...
        # Toggle/activate an object
            ...
            if fwd_cell:
                if hasattr(fwd_cell, 'reward') and fwd_cell.reward is not None:
                    _, reward = fwd_cell.toggle(self, fwd_pos)
                else:
